# HateSlop 4기 엔지니어x프로듀서 합동 캐릭터 챗봇 프로젝트
...
### 가상환경으로 로컬 실행 (Docker 없이)

- Python 3.11 설치되어 있다고 가정합니다.
- 위 Docker 절차에서 `cd 4-chatbot`까지 했다면, 이어서 `**4-base` 디렉터리**로 들어갑니다. (이 `README.md`가 있는 폴더)
...
cp .env.example .env
# .env 파일을 열어 OPENAI_API_KEY(필수) 등을 입력

python app.py
```

- 브라우저: **[http://127.0.0.1:5000](http://127.0.0.1:5000)** (컨테이너의 `5001` 매핑과 달리, 로컬은 기본 포트 **5000**)
- 포트를 Docker와 맞추려면: `PORT=5001 python app.py`
...
> *커스텀 원하시면 수정하셔도 되지만, 의존성을 가진 파일을 같이 수정하셔야 합니다.*
